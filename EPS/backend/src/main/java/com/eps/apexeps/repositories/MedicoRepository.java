            ...
            AND (:diaSemanaParam IS NULL
                OR (t.horario LIKE %:diaSemanaParam%
                    AND (:inicioParam IS NULL
                        OR CAST(SUBSTRING(t.horario, LOCATE(CAST(:diaSemanaParam AS String), t.horario) + 1, 2) AS Integer) <= :inicioParam
                    )
                    AND (:finParam IS NULL
                        OR CAST(SUBSTRING(t.horario, LOCATE(CAST(:diaSemanaParam AS String), t.horario) + 4, 2) AS Integer) >= :finParam
                    )
                )
            )
    """)
